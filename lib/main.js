...
var TEST_REPORT = '%s/job/%s/%s/testReport' + API;
var LAST_TEST_REPORT = '%s/job/%s/lastSuccessfulBuild/testReport' + API;
...
var init = exports.init = function(host, options) {
    ...
    return {
        ...
        last_success: function(jobname, callback) {
            ...
            Get the last build report for a job
            */
            request({method: 'GET', url: build_url(LAST_SUCCESS, jobname)}, function(error, response, body) {
                if ( error || response.statusCode !== 200 ) {
                    callback(error || true, response);
                    return;
                }
        ...
        test_report: function(jobname, build_number, callback) {
          request({method: 'GET', url: build_url(TEST_REPORT, jobname, build_number)}, function(error, response, body) {
              if ( error || response.statusCode !== 200 ) {
                  callback(error || true, response);
                  return;
              }
        ...
        last_test_report: function(jobname, callback) {
            /*
            Get the last test report for a job
            */
            request({method: 'GET', url: build_url(LAST_TEST_REPORT, jobname)}, function(error, response, body) {
                if ( error || response.statusCode !== 200 ) {
                    callback(error || true, response);
                    return;
                }
